...
def cell(arr):
    ...
    for xIndex in range(0,8):
        for yIndex in range(0,64):
            newLayout2.addInstance(arrayCellLayout2,
                                  offsetInMicrons = (xIndex*4.5+28.85,yIndex*6.895-41.5-9.565-1.05),
                                  rotate = 0.0)
...
def dec(arr):
    ...
    for yIndex in range(0,8):
        newLayout.addInstance(arrayCellLayout,
                                offsetInMicrons = (33,yIndex*55.16),
                                rotate = 0.0)
    newLayout.addInstance(arrayCellLayout,
                                offsetInMicrons = (0,0),
                                rotate = 0.0)
    for i in range(0,8):
        newLayout.addBox(layerNumber = 49, offsetInMicrons = (4.0+(i*3),-1.33-(i*6.895)), width = 0.4, height = 387.94-(i*48.265)) 
        newLayout.addBox(layerNumber = 49, offsetInMicrons = (4.0+(i*3),386.33-(i*55.16)), width = (20.27-(i*3)), height = 0.28)
        newLayout.addBox(layerNumber = 49, offsetInMicrons = (3,-1.33-(i*6.895)), width = 1.4 + (i*3), height = 0.21)

    newLayout.addBox(layerNumber = 51, offsetInMicrons = (3,-34.23), width = 27.925, height = 0.28)
    newLayout.addBox(layerNumber = 49, offsetInMicrons = (0.525,-0.5), width = 0.21, height = 301.58+83.51+9.6) 
    newLayout.addBox(layerNumber = 49, offsetInMicrons = (0.525,300.8+83.51+9.6), width = (33.21), height = 0.28)
...
def cir_arr(arr):
    ...
    for xIndex in range(0,8):
        newLayout3.addInstance(arrayCellLayout3,
                                offsetInMicrons = (xIndex*4.5+37.495,-57.5),
                                rotate = 0.0)
    #newLayout.addInstance(dec())
